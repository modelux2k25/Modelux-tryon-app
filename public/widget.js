(function () {
  ...
  console.log('Modelux Widget v5.0 - Zero Config Edition');
  ...
  function loadWidget(container) {
    ...
    const getProductImage = () => {
      ...
      if (container.dataset.productImage) return container.dataset.productImage;

      // 2. Try Open Graph (Standard)
      const ogImage = document.querySelector('meta[property="og:image"]');
      if (ogImage && ogImage.content) return ogImage.content;

      // 3. Try Twitter Card
      const twImage = document.querySelector('meta[name="twitter:image"]');
      if (twImage && twImage.content) return twImage.content;

      // 4. Try Schema.org (JSON-LD)
      ...
      // 5. Try common class names (Last resort)
      const img = document.querySelector('.product-image img, .product-main-image img, #image-main');
    ...
    const renderFloatingWidget = () => {
      const productUrl = getProductImage();
      if (!productUrl) {
        console.log('Modelux: No product image found. Widget disabled.');
        return;
      }
      // Save found image to container for reference
      container.dataset.productImage = productUrl;
      ...
      btn.innerHTML = `
        <img src="${BASE_URL}/modelux-button.webp" alt="Provador Virtual" style="width: 100%; max-width: 250px; height: auto; display: block; margin: 0 auto;">
      `;
      ...
      btn.style.filter = 'drop-shadow(0 4px 6px rgba(0,0,0,0.1))';

      btn.onmouseover = () => btn.style.transform = 'scale(1.02)';
      btn.onmouseout = () => btn.style.transform = 'scale(1)';

      // 2. Try to find the "Buy" button to inject nearby
      const buyButtonSelectors = [
        ...
        '.botao-comprar' // Common in BR themes
      ];

      let buyBtn = null;
      for (const selector of buyButtonSelectors) {
        buyBtn = document.querySelector(selector);
        if (buyBtn && buyBtn.offsetParent !== null) break; // Must be visible
      }

      // 3. Position Logic
      if (buyBtn) {
        // --- INLINE MODE (Above Buy Button) ---
        console.log('Modelux: Buy button found, injecting inline.');
        ...
        buyBtn.parentNode.insertBefore(btn, buyBtn);
      } else {
        // --- FLOATING MODE (Fallback) ---
        console.log('Modelux: Buy button not found, using floating mode.');
        btn.style.position = 'fixed';
        btn.style.bottom = '20px';
        btn.style.right = '20px';
        btn.style.width = 'auto';
        btn.querySelector('img').style.width = '180px'; // Fixed width for floating

        document.body.appendChild(btn);
      }

      // 4. Create Modal Backdrop
  ...
  function init() {
    ...
    if (document.querySelectorAll('.' + CONTAINER_CLASS).length === 0) {
      console.log('Modelux: No container found, auto-injecting.');
