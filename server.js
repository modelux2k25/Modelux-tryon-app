...
app.post('/api/estimate-size', upload.none(), async (req, res) => {
    ...
    `;
        ...
        if (gender === 'Male') {
            if (bmi < 20.5) { size = 'S'; br_size = 'P'; }
            else if (bmi < 25.5) { size = 'M'; br_size = 'M'; }
            else if (bmi < 30.0) { size = 'L'; br_size = 'G'; }
            else { size = 'XL'; br_size = 'GG'; }
        } else {
            // Female
            if (bmi < 19.0) { size = 'S'; br_size = 'P'; }
            else if (bmi < 24.0) { size = 'M'; br_size = 'M'; }
            else if (bmi < 29.0) { size = 'L'; br_size = 'G'; }
            else { size = 'XL'; br_size = 'GG'; }
        }

        // Adjust for height extremes
        if (parseFloat(height) > 190 && size !== 'XL') {
            size = 'L'; br_size = 'G'; // Tall people usually need at least L
            if (bmi > 25) { size = 'XL'; br_size = 'GG'; }
        }

        const fallbackRecommendation = {
            size: size,
            br_size: br_size,
            details: `Sugestão baseada nas suas medidas corporais (Altura: ${height}cm, Peso: ${weight}kg).`,
            fit_note: "Corte padrão estimado."
        };
