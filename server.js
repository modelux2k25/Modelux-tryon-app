...
const generateVirtualTryOn = async (userImageBuffer, userMimeType, productImageUrl, mode = 'clothing') => {
    try {
      ...
      **OUTPUT**:
      ...
      `;
        ...
        const ACCESSORIES_PROMPT = `
      You are an OPTICAL & JEWELRY RETOUCHING EXPERT.
      **CRITICAL MISSION**: Composite the accessory (Glasses, Necklace, Earrings) onto the User with 100% PHOTOREALISM.
      
      **RULES FOR GLASSES (EYEWEAR)**:
      1. **LENS TRANSPARENCY**: This is CRITICAL. If the glasses are clear or lightly tinted, the user's eyes and skin details MUST be visible through the glass. Do NOT make the lenses opaque/solid unless they are black sunglasses.
      2. **REFLECTIONS**: Add subtle, realistic glossy reflections on the lenses and frame to mimic studio lighting.
      3. **DROP SHADOWS**: Cast soft, realistic shadows onto the face (nose bridge, cheeks) where the glasses sit. This prevents the "floating sticker" look.
      4. **FIT**: Ensure the frame sits naturally on the nose bridge and creates depth. 
      
      **RULES FOR JEWELRY**:
      1. **METALLICS**: Gold/Silver must shimmer and reflect the skin tone.
      2. **GRAVITY**: Necklaces must hang naturally, following body contours.
      
      **OUTPUT**: A highly realistic photo. NO CARTOONISH EFFECTS. NO CUTOUT EDGES.
      `;

        const prompt = mode === 'accessories' ? ACCESSORIES_PROMPT : CLOTHING_PROMPT;
...
app.post('/api/generate', upload.single('user_image'), async (req, res) => {
    try {
        ...
        const mode = req.body.mode || 'clothing';
