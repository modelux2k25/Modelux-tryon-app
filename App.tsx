...
export default function App() {
  ...
  const handleCreateAvatar = async () => {
    ...
    try {
      ...
      // Send Mode hint (Clothing vs Accessories)
      formData.append('mode', isLiteMode ? 'accessories' : 'clothing');
    ...
    >
      ...
      <main className="relative z-10 container mx-auto px-4 py-6 max-w-md h-[calc(100vh-80px)] overflow-y-auto pb-20 scrollbar-hide">
        ...
        {step === Step.SIZE_RESULT && (
          <div className="space-y-6 animate-fade-in flex flex-col items-center justify-start h-full pt-4 pb-10">
            ...
            <div className="w-full bg-white rounded-3xl shadow-xl border border-slate-100 p-8 text-center relative overflow-visible">
              <div className="flex flex-col items-center relative z-10">
                <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full uppercase tracking-widest mb-6 border border-emerald-100">Recomendação IA</span>
                ...
                <div className="relative mb-6 group cursor-default">
                  ...
                  <div className="w-32 h-32 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center shadow-lg shadow-emerald-200 relative transform transition-transform group-hover:scale-105">
                    <h1 className="text-6xl font-black text-white tracking-tighter mt-1">{formatSize(sizeRecommendation)}</h1>
                  </div>
                  <div className="absolute -bottom-2 right-0 bg-white p-1.5 rounded-full shadow-md border border-slate-100">
                ...
                <div className="w-full bg-slate-50 rounded-2xl p-4 border border-slate-100">
                  <p className="text-sm text-slate-600 leading-relaxed font-medium">
                    {sizeReasoning || "Calculado com base nas proporções padrão brasileiras e seus dados informados."}
                  </p>
                </div>
              </div>
            </div>
