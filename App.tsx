        ...
        {step === Step.RESULT && resultImage && (
          <div className="space-y-4 animate-fade-in h-full flex flex-col">
            ...
            <div className="flex-1 bg-white rounded-2xl overflow-hidden relative shadow-2xl border border-slate-100 group">
              <img src={resultImage} className="w-full h-full object-cover" alt="Resultado Final" />

              {/* Result Actions Overlay */}
              <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-white via-white/90 to-transparent pt-20 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 hover:opacity-100">
                <button
                  onClick={() => setStep(Step.CHOICE)} // Goes back to choice to maintain context
                  className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl flex items-center justify-center gap-2 shadow-sm border border-slate-200 transition-colors"
                >
                ...
                <a
                  href={resultImage}
                  download={`modelux-tryon-${Date.now()}.png`}
                  className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 transition-colors"
                >
