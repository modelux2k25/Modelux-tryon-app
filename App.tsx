...
export default function App() {
  ...
  const formatSize = (s: string | null) => {
    ...
    // Se tiver no mapa usa, senão retorna o original (ex: '40', '42')
      ...
      <main className="relative z-10 container mx-auto px-4 py-6 max-w-md h-[calc(100vh-80px)] overflow-y-auto pb-20 scrollbar-hide">
        ...
        {step === Step.CHOICE && (
          <div className="space-y-6 animate-fade-in">
            ...
            {/* Banner de Tamanho REMOVIDO pois agora tem tela própria */}
          ...
          <div className="space-y-6 animate-fade-in flex flex-col items-center justify-center h-full pt-10">
            ...
            <div className="w-full max-w-[280px] bg-white rounded-3xl shadow-xl border border-emerald-100 p-8 text-center relative overflow-hidden group">
              ...
              <p className="text-xs text-slate-400 leading-relaxed font-medium">
                {sizeReasoning || "Calculado com base nas proporções padrão brasileiras."}
              </p>
            </div>
            ...
            <div className="space-y-4 bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1 col-span-2"><label className="text-xs font-bold text-slate-500">QUADRIL (cm)</label><input type="number" value={hips} onChange={e => setHips(e.target.value)} className="w-full bg-slate-50 border p-3 rounded-lg" placeholder="100" /></div>
                </div>
              </div>
              <div className="grid grid-cols-1 gap-2 pt-2">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`
                        w-full py-4 rounded-xl font-bold text-white shadow-lg flex items-center justify-center gap-2 relative overflow-hidden group
                        ${isLoading
                      ? 'bg-slate-300 cursor-not-allowed'
                      : 'bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 transform active:scale-[0.98] transition-all'
                    }
                    `}
                >
                  {/* Shine Effect */}
                  {!isLoading && <div className="absolute inset-0 w-full h-full bg-white/20 -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>}
