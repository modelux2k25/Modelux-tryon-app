      ...
      <main className="relative z-10 container mx-auto px-4 py-6 max-w-md h-[calc(100vh-80px)] overflow-y-auto pb-20 scrollbar-hide">
        ...
        {step === Step.RESULT && resultImage && (
          <div className="space-y-6 animate-fade-in h-full flex flex-col">
            ...
            <div className="flex-1 bg-white rounded-2xl overflow-hidden shadow-2xl border border-slate-100 relative">
              <img src={resultImage} className="w-full h-full object-contain bg-slate-50" alt="Resultado Final" />
            </div>

            {/* FIXED: Button OUTSIDE and BELOW image */}
            <div className="pt-2 w-full bg-transparent">
              <button
                onClick={() => setStep(Step.CHOICE)}
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 transition-colors"
              >
            ...
            <div className="text-center text-xs text-slate-400 pb-4">
              Você usou <strong className="text-indigo-600">{dailyUsage}</strong> de {MAX_DAILY_USAGE} tentativas nesta loja.
            </div>
          </div>
        )}
