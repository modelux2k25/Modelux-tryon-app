...
export default function App() {
  ...
  // New state for manual refresh animation
  const [isRefreshingProduct, setIsRefreshingProduct] = useState(false);

  // Size Recommendation State
  const [sizeRecommendation, setSizeRecommendation] = useState<{ size: string; br_size: string; details: string; fit_note: string } | null>(null);
  ...
  useEffect(() => {
    const savedParams = localStorage.getItem('modelux_avatar_params');
    if (savedParams) {
      try {
        setAvatarParams(JSON.parse(savedParams));
      } catch (e) {
  ...
  // Save params when they change
  useEffect(() => {
    if (avatarParams.height) {
      localStorage.setItem('modelux_avatar_params', JSON.stringify(avatarParams));
    }
  }, [avatarParams]);
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'PRODUCT_IMAGE') {
        ...
        setIsRefreshingProduct(false); // Stop animation when done
        setSizeRecommendation(null); // Reset recommendation on product change
      }
      if (event.data && event.data.type === 'PRODUCT_DETAILS') {
        if (event.data.payload.image) setProductImage(event.data.payload.image);
        if (event.data.payload.description) setProductDescription(event.data.payload.description);
      }
    };
  ...
  const handleEstimateSize = async () => {
    ...
    // Check if we have measurements. If not, ask user to create model.
    if (!avatarParams.height || !avatarParams.weight) {
      setErrorMessage('Para sugerir o tamanho, precisamos das suas medidas. Por favor, crie ou refaça seu modelo.');
      return;
    }
    ...
    try {
      ...
      formData.append('hips', avatarParams.hips);
      formData.append('product_image_url', productImage);
      formData.append('product_description', productDescription);

      const response = await fetch('/api/estimate-size', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error('Erro 404: Servidor desatualizado. Faça o deploy do server.js.');
        }
        try {
      ...
      console.error(err);
      setErrorMessage(err.message || 'Não foi possível calcular o tamanho. Tente novamente.');
    } finally {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden relative">
          <button onClick={() => setSizeRecommendation(null)} className="absolute top-3 right-3 text-slate-400 hover:text-slate-600">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>

          <div className="bg-indigo-600 p-6 text-center">
            <p className="text-indigo-100 text-xs font-bold uppercase tracking-wider mb-1">Seu Tamanho Ideal</p>
            <h3 className="text-5xl font-extrabold text-white">{sizeRecommendation.br_size}</h3>
            {sizeRecommendation.size !== sizeRecommendation.br_size && (
              <p className="text-indigo-200 text-sm mt-1">(Intl: {sizeRecommendation.size})</p>
            )}
          </div>
          ...
          <div className="p-6">
            <div className="flex items-start gap-3 mb-4">
              ...
              <p className="text-sm text-slate-600 leading-relaxed">{sizeRecommendation.details}</p>
            </div>

            <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 flex items-center gap-3">
              <svg className="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
              <p className="text-xs font-medium text-slate-700">{sizeRecommendation.fit_note}</p>
            </div>

            <button onClick={() => setSizeRecommendation(null)} className="w-full mt-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors">
              Entendi
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col relative z-0 pt-20 px-6 pb-6 overflow-y-auto custom-scrollbar">
        ...
        {step === Step.CREATE_MODEL && (
          <div className="flex flex-col h-full animate-slide-up">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 p-1 pb-20 custom-scrollbar">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => {
                      ...
                      <svg className={`w-3 h-3 ${isRefreshingProduct ? 'animate-pulse' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" /></svg>
                      {isRefreshingProduct ? 'Clique na foto...' : 'Trocar Peça'}
                    </button>
                  </div>
              ...
              {avatarParams.gender === 'Female' && (
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Quadril (cm)</label>
                  <input type="number" placeholder="95" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-indigo-500 outline-none"
                    value={avatarParams.hips} onChange={e => setAvatarParams({ ...avatarParams, hips: e.target.value })} />
                </div>
              )}
        ...
        {step === Step.UPLOAD && (
          <div className="flex flex-col h-full animate-slide-up">
              ...
              <div className="relative bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm flex flex-col">
                ...
                <div className="absolute top-2 right-2 z-10">
                  <button onClick={() => {
                    setIsRefreshingProduct(true);
                    window.parent.postMessage('START_IMAGE_SELECTION', '*');
                  }} className="bg-white/80 backdrop-blur-sm p-1.5 rounded-full text-indigo-600 shadow-sm hover:bg-white transition-colors" title="Trocar Peça (Clique na foto)">
                    <svg className={`w-3 h-3 ${isRefreshingProduct ? 'animate-pulse' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" /></svg>
                  </button>
                </div>
                <div className="flex-1 p-2 flex items-center justify-center bg-slate-50/50 relative">
            ...
            {/* Size Button (Outside cards) */}
            <button
              onClick={handleEstimateSize}
              disabled={isEstimatingSize}
              className="w-full mt-4 py-3 bg-white border-2 border-indigo-100 rounded-xl text-indigo-600 font-bold text-sm shadow-sm hover:border-indigo-500 hover:bg-indigo-50 transition-all flex items-center justify-center gap-2"
            >
              {isEstimatingSize ? (
            ...
            <div className="mt-4">
              {errorMessage && <div className="mb-4 p-3 bg-red-50 text-red-600 text-xs rounded-lg border border-red-100">{errorMessage}</div>}
              ...
              {/* Button to edit avatar if params exist */}
              {avatarParams.height && (
                <button onClick={() => setStep(Step.CREATE_MODEL)} className="w-full mt-3 py-3 text-sm font-semibold text-purple-600 border border-purple-200 rounded-xl hover:bg-purple-50 transition-colors">
                  Refazer Modelo
                </button>
              )}
            </div>
          ...
          <div className="flex flex-col h-full animate-fade-in">
            ...
            <div className="mt-6 flex flex-col gap-3">
              <button
                ...
                disabled={status === LoadingState.PROCESSING}
                className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all flex items-center justify-center gap-2
                  ${status === LoadingState.PROCESSING ? 'bg-slate-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'}`}
              >
