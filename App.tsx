...
export default function App() {
  ...
  const handleManualRefresh = () => {
    ...
    window.parent.postMessage('REQUEST_PRODUCT_UPDATE', '*');
    // Safety timeout in case no response comes back
    setTimeout(() => setIsRefreshingProduct(false), 2000);
  };
              ...
              <div className="mb-2 p-2 bg-slate-50 border border-slate-100 rounded-xl flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={handleManualRefresh} className="text-[10px] text-indigo-600 font-bold hover:underline flex items-center gap-1">
                      <svg className={`w-3 h-3 ${isRefreshingProduct ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                      {isRefreshingProduct ? 'Buscando...' : 'Atualizar'}
                    </button>
                  </div>
            ...
            <div className="flex-1 grid grid-cols-2 gap-3 sm:gap-4 content-start">
              ...
              <div className="relative bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm flex flex-col">
                ...
                <div className="absolute top-2 right-2 z-10">
                  <button onClick={handleManualRefresh} className="bg-white/80 backdrop-blur-sm p-1.5 rounded-full text-indigo-600 shadow-sm hover:bg-white transition-colors" title="Atualizar Peça">
                    <svg className={`w-3 h-3 ${isRefreshingProduct ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                  </button>
                </div>
