...
export default function App() {
  ...
  const handleEstimateSize = async () => {
    if (!productImage || !avatarParams.height || !avatarParams.weight) {
      setErrorMessage('Para sugerir o tamanho, precisamos dos seus dados (Crie o Modelo primeiro) e da foto do produto.');
      return;
    }
    ...
    try {
      ...
      console.error(err);
      // Don't block the UI, just show error in console or small toast
    } finally {
            ...
            <div className="flex-1 grid grid-cols-2 gap-3 sm:gap-4 content-start">
              ...
              <div className="relative bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm flex flex-col">
                ...
                <div className="flex-1 p-2 flex items-center justify-center bg-slate-50/50 relative">
                  ...
                  {/* Size Recommendation Badge/Button */}
                  <div className="absolute bottom-2 left-0 w-full flex justify-center">
                    {sizeRecommendation ? (
                      <div className="bg-white/90 backdrop-blur-md shadow-lg border border-green-100 rounded-xl p-2 flex items-center gap-3 animate-fade-in max-w-[90%]">
                        <div className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold text-sm border border-green-200">
                          {sizeRecommendation.br_size}
                        </div>
                        <div className="text-left">
                          <p className="text-[10px] font-bold text-slate-700 leading-tight">Tamanho Ideal</p>
                          <p className="text-[9px] text-slate-500 leading-tight">{sizeRecommendation.fit_note}</p>
                        </div>
                      </div>
                    ) : (
                      avatarParams.height && (
                        <button
                          ...
                          className="bg-white/90 backdrop-blur-sm shadow-sm border border-slate-200 rounded-full px-3 py-1.5 text-[10px] font-bold text-indigo-600 flex items-center gap-1.5 hover:bg-indigo-50 transition-colors"
                        >
                          {isEstimatingSize ? (
