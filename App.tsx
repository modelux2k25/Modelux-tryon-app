    ...
    >
      ...
      <main className="relative z-10 container mx-auto px-4 py-6 max-w-md h-[calc(100vh-80px)] overflow-y-auto pb-20 scrollbar-hide">
        ...
        {step === Step.RESULT && resultImage && (
          <div className="h-full flex flex-col animate-fade-in relative pt-0">
            {/* Imagem Expandida */}
            <div className="flex-1 bg-slate-900 rounded-2xl overflow-hidden shadow-2xl relative min-h-0 group">
              <img src={resultImage} className="w-full h-full object-contain" alt="Resultado Final" />

              {/* Botão Fechar Flutuante */}
              <button
                onClick={() => setStep(Step.HOME)}
                className="absolute top-3 right-3 p-2 bg-black/20 hover:bg-black/40 backdrop-blur-md rounded-full text-white/90 transition-all z-10"
              >
                <X className="w-5 h-5" />
              </button>

              {/* Tag "Resultado" sutil */}
              <div className="absolute top-3 left-3 px-3 py-1 bg-black/20 backdrop-blur-md rounded-full text-white/90 text-xs font-medium pointer-events-none">
                Resultado
              </div>
            </div>
